...
tau_th = 7*ms#4*ms#5*ms
Vi = -60*mV#-57*mV 
ka = 5*mV
ki = ka/1.
alpha = ka/ki
Vt = Vi+5*mV#4.*mV#2.5*mV
...
M = StateMonitor(neuron,('V','mu'),record=True)  
...
def STA(T,Vm,lag_max,duration,time_step):
    ...
    deviation = np.std(sample,axis=0) 
...
FigACF.savefig('Fig-pyr-ACF.eps')
FigSTA.savefig('Fig-pyr-STA.eps')